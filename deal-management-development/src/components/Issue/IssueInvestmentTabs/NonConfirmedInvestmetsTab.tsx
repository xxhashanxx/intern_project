...
export interface NonConfirmedInvestmetsTabProps {}
const NonConfirmedInvestmetsTab = (props: NonConfirmedInvestmetsTabProps) => {
  const [selected, setSelected] = React.useState<number[]>([]);
  const [reptype, setReoprtType] = React.useState<string>();
  const issueId = useStoreState((state) => state.issue.issueId);
  const { status, data, error } = useQuery(
    "nonConfirmedQuotationList",
    issueId,
    useinvestmentFilteredListService().useGetQuotationListService
  );
  ...
  const [
    ...
    },
    {
      onSuccess: () => {
        return queryCache
          .refetchQueries("nonConfirmedQuotationList")
          .then(() => {});
      },
      onError: (e) => {
            ...
            <div className="btn-row  flex-end">
              <div className="btn-grid">
                ...
                >
                  ...
                  <option value="QuotationLetter">Quotation Letter</option>
                </Select>
              </div>
              <div className="btn-grid">
                <Button
                  ...
                  disabled={printStatus === "loading" || reptype === undefined}
                >
                          ...
                          {quatationListItem.status === "Preliminary" ? (
                            <Tooltip title="Approve">
                              <IconButton
                                ...
                                color="primary"
                                onClick={() => onApproveClick(quatationListItem)}
                              >
                            ...
                            <Tooltip title="Cancel">
                              ...
                              // style={{ border: "1px solid #546e7a" }}
                                color="default"
                                onClick={() => onCancelClick(quatationListItem)}
                              >
                ...
                <Table size="small" stickyHeader aria-label="sticky table">
                  <TableHead>
                    <TableRow style={{ whiteSpace: "nowrap" }}>
                      ...
                      <TableCell>Approximate Month</TableCell>
                    ...
                    {data.data.map((quatationListItem: any, key: number) => {
                      ...
                      return (
                        <TableRow
                          ...
                          style={{ whiteSpace: "nowrap" }}
                        >
                          <TableCell>{quatationListItem.tranche_no} </TableCell>
                          ...
                          <TableCell>
                            {quatationListItem.approximate_month}{" "}
                          </TableCell>
                          <TableCell align="right">
                            {Number(
                          ...
                          <TableCell align="right">
                            {Number(quatationListItem.interest).toLocaleString(
                              undefined,
                              {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
...
export default NonConfirmedInvestmetsTab;
